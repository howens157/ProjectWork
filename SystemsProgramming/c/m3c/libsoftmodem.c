...
{
    char *tempaudio = malloc(50000 * sizeof(char));
    FILE *message = fmemopen(msg, msglen, "rb");
    FILE *audio = fmemopen(tempaudio, 50000, "wb");
    int adtlen = testTx(8000, 0.0, message, audio);
    *adt = malloc(adtlen * sizeof(char));
    memcpy(*adt, tempaudio, adtlen); 
...
{
    char *tempmsg = malloc(50000 * sizeof(char));
    memset(tempmsg, '\0', 50000);
    FILE *audio = fmemopen(adt, adtlen, "rb");
    FILE *message = fmemopen(tempmsg, 50000, "wb");
    int nmglen = testRx(8000, audio, message);
    *msg = malloc(nmglen * sizeof(char));
    memset(*msg, '\0', nmglen);
    memcpy(*msg, tempmsg, nmglen);
